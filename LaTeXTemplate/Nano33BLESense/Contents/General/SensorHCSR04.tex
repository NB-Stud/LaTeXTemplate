...
%Nachfolgend ein vereinfachter Arduino-Sketch für ein Projekt, das den
...
\python{pingMedian()}: Durchführung mehrerer Messungen und Rückgabe des Medianwertes zur Reduktion von Ausreißern.\\
	%\item \textit{convert()}: Umrechnung einer bekannten Echozeit in Zentimeter zur manuellen Berechnung der Entfernung.
	%\item \textit{lastDistance()}: Zugriff auf das letzte Messergebnis, ohne einen neuen Impuls auszulösen.
...
Dieses Arduino-Programm \ref{lst:schwellenwert} nutzt einen HC-SR04 Ultraschallsensor, um die Entfernung zu einem Objekt kontinuierlich zu überwachen. Es erkennt, ob eine kritische Schwelle (25 cm) unterschritten wurde und gibt in diesem Fall eine Warnmeldung im Seriellen Monitor aus.

\begin{listing}
	\caption{Einfache Anwendung als Schwellenwert-Melder}
	\label{lst:schwellenwert}
...
Sense Lite) angeschlossen und ein einfacher Sketch  \ref{lst:entfernungMessung} aufgespielt, welcher die gemessene
