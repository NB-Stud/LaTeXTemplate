...
\chapter{Projekt: Magic Faucet Fountain}\label{beschreibung}


\section{Beschreibung}
Das Projekt Magic Faucet Fountain beschäftigt sich mit der Umsetzung eines Brunnens, der die optische Täuschung eines frei schwebenden Wasserhahns erzeugt. Dabei scheint es, als würde Wasser kontinuierlich aus einem frei schwebenden Wasserhahn fließen - ganz ohne sichtbare Verbindung zur Wasserquelle \ref{MagicFaucet}.
\begin{figure}[H]
	\centering
...
Komponenten dieser Schaltung sind: 3x 5 V-Relais, LED-Leuchten (grün und blau) und die Wasserpumpe. Dem Aufbau der Schaltung ist zu entnehmen, dass jedes Relais als Schalter arbeitet, das über den Arduino gesteuert wird. Die genaue Ansteuerung der Komponenten durch den Arduino wird in Tabelle \ref{AnsteuerungArduino} dargestellt.
	\begin{center}
	\captionof{table}{Ansteuerung der Komponenten durch den Arduino}
	\begin{tabular}{c|c|c|c}
		Relais & Gesteuertes Element & Arduino Pin & Versorgungsspannung\\ \hline
		 1 & LED grün & D4 & 5 V\\
	     2 & LED blau & D3 & 5 V\\
...
\begin{itemize}
	...
	\begin{itemize}
        	...
        	\item Mehrere Rücklaufbohrungen, durch die Wasser in den Blumentopf zurückfließt.
	\end{itemize}
...
Nach erfolgreicher Installation der App \textit{nRF Connect}, wie in Kapitel \ref{chap:einstiegNRF} beschrieben, kann man sich mit dem Magic Faucet Fountain Verbinden. Wenn Sie den Kippschalter betätigen, fängt die grüne LED an zu leuchten und die blaue LED blinkt. Das Signalisiert, dass die Versorgungsspannung vorhanden ist (grün) und dass der Magic Faucet Fountain nach BLE-Geräte in der Nähe sucht (blau). 

\subsection*{Verbindung mit der Applikation herstellen}
Zuerst öffnet man die \textit{nRF Connect}-App und aktualisiert den \menu{Scanner} durch ein nach unten Wischen oder ein Tippen auf den Bildschirm. Danach sollte der Magic Faucet Fountain in der BLE-Geräteansicht auftauchen. Drücken Sie dann auf \menu{Connect} um sich mit dem Magic Faucet Fountain zu verbinden. Wenn Sie sich erfolgreich verbunden haben, leuchtet die blaue LED konstant und signalisiert eine stabile Verbindung. \ref{fig:connectNRF}

\begin{figure}[H]
	\centering
	\begin{minipage}[t]{0.3\textwidth}
		\centering
		\includegraphics[width=\linewidth]{nRF Connect/scanApplikation.png}
		\caption*{1. BLE-Geräte Scannen}
	\end{minipage}
	\hfill
	\begin{minipage}[t]{0.3\textwidth}
		\centering
		\includegraphics[width=\linewidth]{nRF Connect/connectedApplikation.png}
		\caption*{2. Auf \menu{Connect} tippen}
	\end{minipage}
	\caption{Scannen und hinzufügen des Magic Faucet Fountain}
	\hfill
	\label{fig:connectNRF}
\end{figure}

\subsection*{App-Ansicht}
Wenn Sie nun auf den Magic Faucet Fountain tippen, erscheint der \menu{Attribute Table}, wie in \ref{fig:attributeApplikation} zu sehen, dort können Sie den Magic Faucet Fountain steuern, den Wasserpegel ablesen und Warnmeldungen erhalten. 

\begin{figure}[H]
	\caption{Die einstellbaren Attribute des Magic Faucet Fountain}
	\centering
	\label{fig:attributeApplikation}
	\includegraphics[width=0.3\textwidth]{nRF Connect/attributeApplikation.png}
\end{figure}

Um die Attribute zu aktivieren, tippen Sie auf das $\downarrow$-Symbol ganz rechts und wählen Sie als Data Parser \menu{UTF-8} aus \ref{fig:dataApplikation}. Wiederholen Sie den Vorgang bei jedem Attribut:

\begin{figure}[H]
	\caption{Aktivieren der Attribute}
	\centering
	\label{fig:dataApplikation}
	\includegraphics[width=0.3\textwidth]{nRF Connect/dataApplikation.png}
\end{figure}

Nachdem sie alle Attribute aktiviert haben, sollte ihre App-Ansicht so aussehen: \ref{fig:data2Applikation}
\begin{figure}[H]
	\caption{Aktivierte Attribute}
	\centering
	\label{fig:data2Applikation}
	\includegraphics[width=0.3\textwidth]{nRF Connect/data2Applikation.png}
\end{figure}

Anhand der UUID lassen sich die Attribute unterscheiden:
\begin{itemize}
	\item \menu{19B10000}: Haupt BLE-Service, stellt die Kommunikation her
	\item \menu{19B10001}: Steuert die Pumpe 
	\item \menu{19B10002}: Zeigt den Wasserstand an
	\item \menu{19B10004}: Statusmeldungen
\end{itemize}

\subsection*{Steuerung des Magic Faucet Fountain}
Möchten Sie nun die Pumpe an- oder ausschalten, tippen Sie einfach bei \menu{19B10001} auf das $\uparrow$-Symbol. Sie gelangen ins \menu{Write Value}-Menü. Dort stellen Sie \menu{UTF-8} ein und können entweder \glqq An \grqq oder \glqq Aus \grqq schreiben. Die Pumpe wird dann jeweils an- oder ausgeschaltet werden. \ref{fig:steuerungNRF}

\begin{figure}[H]
	\centering
	\begin{minipage}[t]{0.3\textwidth}
		\centering
		\includegraphics[width=\linewidth]{nRF Connect/anApplikation.png}
		\caption*{\glqq An\grqq}
	\end{minipage}
	\hfill
	\begin{minipage}[t]{0.3\textwidth}
		\centering
		\includegraphics[width=\linewidth]{nRF Connect/ausApplikation.png}
		\caption*{\glqq Aus\grqq}
	\end{minipage}
	\caption{Steuerung der Pumpe durch Schreiben von \glqq An \grqq oder \glqq Aus \grqq}
	\hfill
	\label{fig:steuerungNRF}
\end{figure}
