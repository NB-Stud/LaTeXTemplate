...
% $ Pfad: LaTeXTemplate/Nano33BLESense/Contents/Nano33BLESense/Nano33BLESense.tex $
% $ Kurzbeschreibung: In diesem Dokument geht es um den Arduino Nano 33 BLE Sense. Es folgt eine Beschreibung und Aufbau des Boards und eine Auflistung und Erklärung der integrierten Sensoren. Desweiteren wird auf die Unterschiede der verschiedenen Versionen Rev1 und Rev2 eingegangen und die Pinbelegung erklärt. $
%
...
\chapter{Arduino Nano 33 BLE Sense}\label{NanoBLESense}
...
Der Mikrocontroller - oft als \glqq Mini-Computer\grqq des Arduino-Boards bezeichnet - benötigt normalerweise verschiedene elektronische Bauteile wie Dioden, Widerstände, Kondensatoren und Transistoren zur Spannungs- und Stromregulierung. Der Arduino beinhaltet bereits viele elektronische Bauteile und ermöglicht einen einfachen Einstieg in die Welt der Mikrocontroller. Folgende Vorteile bietet der Arduino:
...
\begin{itemize}
	...
	\end{itemize}
	\item Energieeffizientem Design im Nano-Format (45x18 mm)
\end{itemize}

\subsection*{Namenskonvention}
Die Bezeichnung gibt Aufschluss über die Funktionen:
\begin{itemize}
	\item \textbf{Nano}: kompakte Bauform (45x18 mm)
	\item \textbf{33}: Betriebsspannung 3.3 V
...
\end{tikzpicture}  
    \captionof{figure}{Arduino Nano 33 BLE Sense Rev. 2, siehe \href{https://store.arduino.cc/arduino-nano-33-ble-sense}{Arduino Store}} 
    \label{ArduinoNano33}
\end{center}

\subsection*{Technische Spezifikationen des Arduino Nano 33 BLE Sense}

Das kompakte und zuverlässige BLE-Board basiert auf dem NINA-B306-Modul für Bluetooth Low Energy (BLE 5.0). Dieses Modul enthält den leistungsstarken Nordic nRF52480-Prozessor mit Cortex-M4F-CPU und ist vollständig kompatibel mit der Arduino IDE (online und offline). \cite{ArduinoWeb:2025}
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item Reichweite bis zu 100 m (freies Feld)
		\item Datenrate bis zu 2 Mbit/s
	\end{itemize}
	...
	\begin{itemize}
		...
		\item Frequenzbereich: 100 Hz-16 kHz
...
Die Unterschiede zwischen dem Arduino Nano 33 BLE Sense und der Rev2-Version betreffen folgende Komponenten: IMU, Temperatur- und Feuchtigkeitssensor, Mikrofon, Krypto-Chip und Spannungswandler. In der zweiten Revision wurde:
...
\begin{itemize}
	...
	\item \textbf{Kompensation}: Externe Feuchtigkeitssensoren können über I²C angeschlossen werden
...
\begin{itemize}
	...
	\item Der LSM9DS1 ist eine 9-Achsen-Inertialmesseinheit (IMU) (Beschleunigungsmesser, Gyroskop und Magnetometer), ideal für Wearables.
...
\begin{center}
	...
	\end{tikzpicture}
	\captionof{figure}{Anordnung der Sensoren auf dem Board}\label{ArduinoNano33BLESenseArchitecture}
\end{center}
...
\begin{table}
	\begin{center}
		\begin{tabular}{llm{90mm}} 
			...
			\textbf{\MapleCommand{OPERATING VOLTAGE}}  & 3.3 V\\
			\textbf{\MapleCommand{INPUT VOLTAGE (LIMIT)}}  & 21 V \\
			\textbf{\MapleCommand{DC CURRENT PER I/O PIN}}  & 15 mA \\
			\textbf{\MapleCommand{CLOCK SPEED}} & 64 MHz \\
			\textbf{\MapleCommand{CPU FLASH MEMORY}}  & 1 MB  \\
			\textbf{\MapleCommand{SRAM}}  & 256 KB  \\
		...
		\end{tabular}
	\end{center}
	\caption{Technische Spezifikation des Arduino Nano 33 BLE Sense \cite{ArduinoWeb:2025}}
\end{table}
...
\begin{center}
	\begin{tikzpicture}
		...
		\node[rotate=90,left] (Pin3) at (9.1,-0.0) {+5 V \; (27)};
...
\begin{description}
	\item[Digitale Pins:] 14 digitale I/O-Pins, die nur die Zustände HIGH oder LOW erkennen. Diese Pins können je nach Anforderung als Eingang oder Ausgang konfiguriert werden. Bei 5 V liegen sie im HIGH-Zustand, bei 0 V im LOW-Zustand.
	\item[Analoge Pins:] 8 analoge Eingänge (A0 bis A7), die im Gegensatz zu digitalen Pins kontinuierliche Werte erfassen können. Der Messbereich liegt zwischen 0-5 V.
	...
	\item[Besondere Pins:] Folgende Pins:
